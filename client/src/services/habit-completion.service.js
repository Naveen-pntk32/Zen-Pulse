...
const habitCompletionService = {
  async getAllForDate(date) {
    if (getBackend() === "supabase") {
      const { data, error } = await supabase.from("habit_completions").select("*").eq("date", date);
      if (error) throw error;
      return (data ?? []).map(mapRow);
    }
    return localDb.getHabitCompletions().filter((c) => c.date === date);
  },
  async getAllBetween(from, to) {
    if (getBackend() === "supabase") {
      const { data, error } = await supabase.from("habit_completions").select("*").gte("date", from).lte("date", to);
      if (error) throw error;
      return (data ?? []).map(mapRow);
    }
    return localDb.getHabitCompletions().filter(
      (c) => c.date >= from && c.date <= to
    );
  },
  async get(habitId, date) {
    if (getBackend() === "supabase") {
      const { data, error } = await supabase.from("habit_completions").select("*").eq("habit_id", habitId).eq("date", date).maybeSingle();
      if (error) throw error;
      return data ? mapRow(data) : null;
    }
    return localDb.getHabitCompletions().find(
      (c) => c.habitId === habitId && c.date === date
    ) ?? null;
  },
  async upsert(completion, userId) {
    if (getBackend() === "supabase") {
      const payload = {
      ...
      const { data, error } = await supabase.from("habit_completions").upsert(payload, { onConflict: "habit_id,date" }).select().single();
      if (error) throw error;
      return mapRow(data);
    }
    const completions = localDb.getHabitCompletions();
  ...
  async toggleManual(habitId, date, userId) {
    ...
    return this.upsert(
      {
        ...
        completedAt: nowCompleted ? (/* @__PURE__ */ new Date()).toISOString() : null
      },
      userId
    );
  },
  /** Return the actual focused seconds for a habit on a date. */
  async getActualFocusSeconds(habitId, date) {
   ...
   */
  async evaluateAutoCheckIn(session, userId) {
...
function useToggleHabitCompletion() {
  ...
  return useMutation({
    mutationFn: ({ habitId, date }) => habitCompletionService.toggleManual(habitId, date, getCurrentUserIdSync() ?? "local"),
