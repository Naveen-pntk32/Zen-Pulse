...
function TasksPage({ params }) {
  const listId = params?.listId;
  const isAll = !listId;
  const { data: lists } = useTaskLists();
  const { data: tasks, isLoading } = useTasks(isAll ? void 0 : { listId });
      ...
      <div className="flex-1 min-w-0">
        <div className="mb-6">
          <h1 className="text-2xl font-bold flex items-center gap-2">
            {isAll ? "All Tasks" : <>
                <span>{currentList?.icon || "\u{1F4CB}"}</span>
                {currentList?.name ?? "Tasks"}
              </>}
          </h1>
          <p className="text-gray-400 text-sm mt-1">
            {isAll ? "All your tasks across every list" : `Tasks in the ${currentList?.name ?? "this"} list`}
          </p>
          {!isAll && <div className="mt-2 text-xs text-gray-500 flex items-center gap-1">
        ...
        {isLoading ? <div className="flex justify-center py-16">
          ...
          </div> : (tasks ?? []).length === 0 ? <div className="text-center text-gray-500 py-16 space-y-2">
            <Inbox className="w-12 h-12 mx-auto opacity-40" />
            <p>No tasks here yet</p>
            <p className="text-sm">Add a task to get started</p>
          </div> : <div className="space-y-2.5">
            {pending.map((task) => <TaskItem key={task.id} task={task} listIcon={currentList?.icon} />)}
            {completed.length > 0 && <div className="pt-3">
                <div className="text-xs font-medium text-gray-500 mb-2 uppercase tracking-wider">
                  Completed ({completed.length})
                </div>
                {completed.map((task) => <TaskItem key={task.id} task={task} listIcon={currentList?.icon} />)}
              </div>}
          </div>}
