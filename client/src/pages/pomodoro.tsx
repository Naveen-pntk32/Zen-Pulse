...
export default function PomodoroPage() {
  ...
  const statusLabel = displayTarget
    ? displayMode === 'pomodoro'
      ? `Focus: ${displayTarget.name}`
      : `Stopwatch · ${displayTarget.name}`
    : displayMode === 'pomodoro'
      ? 'Focus Time'
      : 'Stopwatch';

  const completedPomodorosToday =
    todaySessions?.filter((s) => s.type === 'pomodoro' && s.status === 'completed').length ?? 0;
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1fr_380px] gap-6 items-start">
        ...
        <div className="bg-gray-800 rounded-3xl border border-gray-700 p-6 flex flex-col items-center space-y-6">
          ...
          <div className="flex items-center justify-between w-full">
            <TimerSettings />
            <div className="text-xs text-gray-500">{displayMode === 'pomodoro' ? 'Skip 🔄 Reset' : 'Reset'}</div>
          </div>
