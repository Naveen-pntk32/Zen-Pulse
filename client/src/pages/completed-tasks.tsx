...
import { CompletedFilters, DateFilterValue } from '@/components/tasks/CompletedFilters';
...
function inDateRange(completedAt: string | null, filter: DateFilterValue, from: string, to: string): boolean {
  if (!completedAt) return false;
  const key = completedAt.slice(0, 10);
  if (filter === 'all') return true;
  if (filter === 'today') return key === getLocalDateKey();
  if (filter === '7') {
    const cutoff = getLocalDateKey(addDays(new Date(), -6));
    return key >= cutoff;
  }
  if (filter === '30') {
    const cutoff = getLocalDateKey(addDays(new Date(), -29));
    return key >= cutoff;
  }
  if (filter === 'custom') {
...
export default function CompletedTasksPage() {
  ...
  const groups = useMemo(() => {
    ...
    return Array.from(byDate.entries())
      .sort((a, b) => parseLocalDateKey(b[0]).getTime() - parseLocalDateKey(a[0]).getTime());
        ...
        {isLoading ? (
          ...
          <div className="space-y-6">
            {groups.map(([date, dateTasks]) => (
              <CompletedTaskGroup key={date} date={date} tasks={dateTasks} lists={lists ?? []} />
            ))}
          </div>
        )}
