...
export function FocusTargetSelector({
  value,
  onChange,
  disabled,
}: {
  value: FocusTarget | null;
  onChange: (target: FocusTarget | null) => void;
  disabled?: boolean;
}) {
  ...
  return (
    <Select
      ...
      onValueChange={(v) => {
        ...
        onChange({ type: type as 'task' | 'habit', id, name: name ?? '' });
      }}
      disabled={disabled}
    >
        ...
        {loading ? (
          ...
          <>
            <SelectGroup>
              ...
              {(habits ?? []).map((h) => (
                <SelectItem key={`habit:${h.id}`} value={`habit:${h.id}`}>
                  {h.icon} {h.name}
                </SelectItem>
              ))}
            </SelectGroup>
