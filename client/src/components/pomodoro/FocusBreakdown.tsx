...
export function FocusBreakdown({ items }: { items: BreakdownItem[] }) {
    ...
    <div className="space-y-3">
      {items.map((item, index) => {
        const icon =
          item.icon ||
          (item.type === 'habit'
            ? habits?.find((h) => h.name === item.targetName)?.icon || '😊'
            : tasks?.find((t) => t.title === item.targetName)?.title
              ? '📋'
              : '🎯');
        const width = Math.max(6, (item.seconds / maxSeconds) * 100);
        return (
          <div key={`${item.type}:${item.targetName}`} className="space-y-1">
