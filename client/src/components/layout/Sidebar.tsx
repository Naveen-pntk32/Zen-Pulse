    ...
    <div className="flex flex-col h-full">
      ...
      <nav className="flex-1 px-3 space-y-1">
        ...
        <Link
          ...
          onClick={onNavigate}
          className={cn(
            'flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-colors opacity-60',
            location === '/music'
              ? 'bg-indigo-500/20 text-indigo-300'
              : 'text-gray-400 hover:text-white hover:bg-gray-700/60',
          )}
...
export function Sidebar() {
  return (
    <aside className="hidden md:flex w-64 shrink-0 bg-gray-900 border-r border-gray-800">
      <SidebarContent />
    </aside>
  );
}
...
export function MobileHeader({ onMenuOpen }: { onMenuOpen: () => void }) {
  return (
    <header className="md:hidden sticky top-0 z-30 flex items-center justify-between px-4 py-3 bg-gray-900/95 backdrop-blur border-b border-gray-800">
      <button
        onClick={onMenuOpen}
