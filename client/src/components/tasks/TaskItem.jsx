...
function TaskItem({
  ...
  const handleToggle = () => {
    updateTask.mutate({
      ...
      patch: {
        ...
        completedAt: task.status === "completed" ? null : (/* @__PURE__ */ new Date()).toISOString()
      }
    });
  };
  const saveEdit = () => {
  ...
  const dueLabel = task.dueDate ? (/* @__PURE__ */ new Date(`${task.dueDate}T${task.dueTime ?? "00:00"}`)).toLocaleDateString("en-US", {
    month: "short",
    day: "numeric",
    year: "numeric"
  }) : null;
  return <div
    className={cn(
      ...
      task.status === "completed" ? "bg-gray-800/40 border-gray-700/50 opacity-70" : "bg-gray-800 border-gray-700 hover:border-gray-600"
    )}
  >
  ...
  >
          ...
          <div className="flex flex-wrap items-center gap-2 mt-1.5 text-xs text-gray-400">
            ...
            {dueLabel && <span className="flex items-center gap-1">
