...
function TaskListSidebar() {
  ...
  const currentListId = location.startsWith("/tasks/") && !location.startsWith("/tasks/completed") ? location.split("/")[2] : null;
  const isCompleted = location.startsWith("/tasks/completed");
  const isAll = location === "/tasks";
  return <div className="w-full md:w-56 shrink-0 md:border-r md:border-gray-800 md:pr-4">
  ...
  >
        ...
        {isLoading ? <div className="flex justify-center py-4">
          ...
          </div> : (lists ?? []).map((list) => <Link
    key={list.id}
    href={`/tasks/${list.id}`}
    className={cn(
      "flex items-center gap-2.5 px-3 py-2 rounded-xl text-sm font-medium transition-colors",
      currentListId === list.id ? "bg-indigo-500/20 text-indigo-300" : "text-gray-400 hover:text-white hover:bg-gray-700/60"
    )}
  >
              <span className="text-base leading-none">{list.icon || "\u{1F4CB}"}</span>
