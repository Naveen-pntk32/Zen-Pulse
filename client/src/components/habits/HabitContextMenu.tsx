...
function EditHabitDialog({ habit, open, onOpenChange }: { habit: Habit; open: boolean; onOpenChange: (o: boolean) => void }) {
  const updateHabit = useUpdateHabit();
  const [name, setName] = useState(habit.name);
  const [section, setSection] = useState<HabitSection>(habit.section);
  const [goalSeconds, setGoalSeconds] = useState(habit.dailyGoalSeconds || 10800);
  const [goalType, setGoalType] = useState<'time' | 'simple'>(habit.goalType === 'simple' ? 'simple' : 'time');
      ...
      <DialogContent className="bg-gray-800 border-gray-700 text-gray-100 max-w-sm">
        ...
        <div className="space-y-4 py-2">
          ...
          <div className="space-y-2">
            ...
            <Select value={section} onValueChange={(v) => setSection(v as HabitSection)}>
...
export function HabitContextMenu({ habit, children }: { habit: Habit; children: React.ReactNode }) {
  const [, navigate] = useLocation();
  const deleteHabit = useDeleteHabit();
  const updateHabit = useUpdateHabit();
  const [editOpen, setEditOpen] = useState(false);

  const startFocus = (mode: 'pomodoro' | 'stopwatch') => {
